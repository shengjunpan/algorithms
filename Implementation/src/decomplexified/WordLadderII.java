            ...
            for (String word : currentLevel) {
                for (int i = 0; i < word.length(); ++i) {
                    ...
                    for (chars[i] = 'a'; chars[i] <= 'z'; ++chars[i]) {
                        if (chars[i] == word.charAt(i)) { continue; }
...
//end{find-ladders}
    ...
    private static void testBuildGraph() {
        ...
        HashMap<String, HashSet<String>> parents = solver.buildGraph("cog", "hit", dict);
        for (String word : parents.keySet()) {
            System.out.println(word + " --> " + parents.get(word));
        }
    }
